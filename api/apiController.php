...
function obtenirObjectes($db, $categoria = null) {
    // Consulta base comuna per a tots dos casos
    ...
    if ($categoria !== null) {
        // Si hi ha categoria, filtrem de manera segura amb un Statment preparat
        $sql .= " WHERE c.cat_nom = :cat ORDER BY o.obj_id DESC";
        $stmt = $db->prepare($sql);
        $stmt->bindValue(':cat', $categoria, SQLITE3_TEXT);
        return $stmt->execute();
    } else {
        // Si no hi ha categoria, ho ordenem i executem directament
...
/**
 * Obté el llistat de totes les categories disponibles.
 */
