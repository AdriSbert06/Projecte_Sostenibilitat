<?php
    ...
    function obtenirObjectesGestio($db, $id_usuari, $es_admin) {
        if ($es_admin) {
            $sql = "SELECT o.*, c.cat_nom, u.usu_nom AS proprietari_nom 
                    FROM objectes o 
                    JOIN categories c ON o.cat_id = c.cat_id
                    JOIN usuaris u ON o.usu_propietari_id = u.usu_id
            ...
            $sql = "SELECT o.*, c.cat_nom, 'Jo' AS proprietari_nom 
                    FROM objectes o 
                    JOIN categories c ON o.cat_id = c.cat_id 
                    WHERE o.usu_propietari_id = :usu_id 
                    ORDER BY o.obj_id DESC";
            $stmt = $db->prepare($sql);
            $stmt->bindValue(':usu_id', $id_usuari, SQLITE3_INTEGER);
            return $stmt->execute();
        }
    }

    function obtenirCategoriesCompletes($db) {
        return $db->query("SELECT * FROM categories");
    }

    function obtenirObjectePerId($db, $id) {
        ...
        $stmt->bindValue(':id', $id, SQLITE3_INTEGER);
        $exec = $stmt->execute();
        return $exec ? $exec->fetchArray(SQLITE3_ASSOC) : false;
    }
?>